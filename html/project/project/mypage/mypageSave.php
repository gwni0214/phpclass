        ...
        if($result){
            ...
            if($memberInfo['memberID'] == $memberID){
                ...
                if($fileType == "image"){
                    ...
                    $sql = "UPDATE myProject SET youEmail = '{$youEmail}', youName = '{$youName}', youBirth = '{$youBirth}', youPhone = '{$youPhone}', youPass = '{$youPass}', youNickname = '{$youNickname}', youPhoto = 'default.svg' WHERE memberID = '{$memberID}'";
                    $connect -> query($sql);
                    $result = $connect -> query($sql);
                    $result = move_uploaded_file($myPageTmp, $myPageDir.$myPageName);
                    }               
